...
impl MockAptosDataClient {
    ...
    fn emulate_network_request_timeout(&self) -> aptos_data_client::error::Error {
        // Sleep for a while
    ...
    /// Verifies the request timeout for the given request type
...
impl AptosDataClientInterface for MockAptosDataClient {
    ...
    async fn get_state_values_with_proof(
        ...
        self.verify_request_timeout(request_timeout_ms, false, false, data_request);
    ...
    > {
        ...
        self.verify_request_timeout(request_timeout_ms, true, false, data_request);
        ...
        if self.data_beyond_highest_advertised && known_version < MAX_REAL_TRANSACTION_OUTPUT {
            ...
            let target_ledger_info =
                determine_targer_ledger_info(known_epoch, request_timeout_ms, &aptos_data_client)
                    .await;
        ...
        Err(self.emulate_network_request_timeout())
    }
    ...
    > {
        ...
        self.verify_request_timeout(request_timeout_ms, false, true, data_request);
...
async fn determine_targer_ledger_info(
    known_epoch: Epoch,
    request_timeout_ms: u64,
