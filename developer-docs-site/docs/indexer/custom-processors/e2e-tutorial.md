...
Processors consume a stream of transactions from the Transaction Stream Service. In order to use the Labs-Hosted Transaction Stream Service you need an auth token. Follow [this guide](../txn-stream/labs-hosted#auth-tokens) to guide to get one. Once you're done, you should have a token that looks like this:
```
